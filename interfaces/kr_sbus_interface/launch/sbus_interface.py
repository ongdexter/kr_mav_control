...
def generate_launch_description():
    ...
    config_file = os.path.join(
        get_package_share_directory('kr_sbus_interface'),
        'config',
        'neurofly.yaml'
    )

    # Component container to load your SO3CmdToSbus component
    sbus_container = ComposableNodeContainer(
        name='so3cmd_to_sbus_container',
        namespace=robot_ns,
        package='rclcpp_components',
        executable='component_container_mt',  # multi-threaded
        composable_node_descriptions=[
            ComposableNode(
                package='kr_sbus_interface',
                plugin='SO3CmdToSBUS',
                name='so3cmd_to_sbus',
                remappings=[
                    ('odom',   odom_topic),
