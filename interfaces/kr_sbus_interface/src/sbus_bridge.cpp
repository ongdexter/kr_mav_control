...
//   sbus_cmd_pub_ = node_->create_publisher<nav_msgs::msg::Odometry>("sbus_cmd", 1);
  ...
  try {
    ...
    RCLCPP_ERROR(
        logger_,
        "Could not successfully start watchdog thread. Exiting SBusBridge.");
    rclcpp::shutdown();
    return;
  }
    ...
    const Eigen::Quaterniond &odom_q) 
{
  ...
  {    
        ...
        bridge_state_ != BridgeState::RC_FLIGHT)
    {
      RCLCPP_WARN(
          logger_,
          "Received active control command but sbus bridge is not armed. "
...
SBusMsg SBusBridge::generateSBusMessageFromSO3Command(const kr_mav_msgs::msg::SO3Command::ConstPtr& msg, const Eigen::Quaterniond& odom_q) const
{
  SBusMsg sbus_msg;
...
bool SBusBridge::loadParameters()
{
  ...
  node_->declare_parameter("enable_receiving_sbus_messages", true);
