...
{
  channels[sbus_channel_mapping::kThrottle] = throttle_cmd;
}

void SBusMsg::setRollCommand(const uint16_t roll_cmd)
{
  channels[sbus_channel_mapping::kRoll] = roll_cmd;
}

void SBusMsg::setPitchCommand(const uint16_t pitch_cmd)
{
  channels[sbus_channel_mapping::kPitch] = pitch_cmd;
}

void SBusMsg::setYawCommand(const uint16_t yaw_cmd)
{
  channels[sbus_channel_mapping::kYaw] = yaw_cmd;
}
...
void SBusMsg::setControlMode(const ControlMode &control_mode)
{
  ...
  {
    channels[sbus_channel_mapping::kControlMode] = kMinCmd;
  }
  else if(control_mode == ControlMode::BODY_RATES)
  {
    channels[sbus_channel_mapping::kControlMode] = kMaxCmd;
  }
}
...
void SBusMsg::setArmState(const ArmState &arm_state)
{
  ...
  {
    channels[sbus_channel_mapping::kArming] = kMaxCmd;
  }
  else
  {
    channels[sbus_channel_mapping::kArming] = kMinCmd;
  }
}

void SBusMsg::setArmStateArmed()
{
...
{
  if(channels[sbus_channel_mapping::kArming] <= kMeanCmd)
  {
    return false;
  }
...
{
  if(channels[sbus_channel_mapping::kKillSwitch] > kMeanCmd)
  {
    return false;
  }
...
{
  if(channels[sbus_channel_mapping::kControlMode] > kMeanCmd)
  {
