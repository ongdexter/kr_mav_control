...
bool SBusSerialPort::startReceiverThread() {
  ...
  try {
    ...
    RCLCPP_ERROR(logger_, "Could not successfully start SBUS receiver thread.");
    return false;
  }
...
void SBusSerialPort::transmitSerialSBusMessage(const SBusMsg& sbus_msg) const {
  ...
  if (written != kSbusFrameLength_) {
    RCLCPP_ERROR(logger_, "Wrote %d bytes but should have written %d", written, kSbusFrameLength_);
  }
}
      ...
      if (fds[0].revents & POLLIN) {
        ...
        while (bytes_buf.size() >= kSbusFrameLength_) {
          ...
          if (bytes_buf.front() == kSbusHeaderByte_ &&
            ...
            bytes_buf.pop_front();
            RCLCPP_WARN(logger_, "SBUS message framing not in sync");
          }
